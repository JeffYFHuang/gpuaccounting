...
public interface NamespaceusedresourcequotaRepository extends JpaRepository<Namespaceusedresourcequota, Long> {
	@Query(value = "SELECT u FROM Namespaceusedresourcequota u WHERE u.queryTime > :startDateTime AND u.queryTime < :endDateTime")
	List<Namespaceusedresourcequota> findAll(
			@Param("startDateTime") String startDateTime, 
			@Param("endDateTime") String endDateTime);

	@Query(value = "SELECT u FROM Namespaceusedresourcequota u WHERE u.namespaceId = :namespaceId AND u.queryTime > :startDateTime AND u.queryTime < :endDateTime")
